...
## What's new for this mid-life update release
 ...
 7. `battery-voltage` now uses `libiio`.
 8. Switched geolocation provider to `ip-api.com`.`
 9. We make building `nodejs` optional, as not many seem to be using that these days and it takes half the build time. However, for now the binary images will be built with `nodejs` included.
 10. We are dropping support for `ext4` images. `flashall` will make `btrfs` the default. The `btrfs` image will also work if loaded from the `sdhc`.
 11. When calling `make recover` Yocto will build the `xfstk-native` recipe then call `flashall.sh --recovery --native`. This ensures `xfstk` is available on your platform automatically.
