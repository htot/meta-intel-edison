...
permalink: 0.7-Why-btrfs-is-great.html
sidebar: edison
product: Edison
---

The original Intel image for Intel Edison-Arduino had multiple partition using a combination of file systems (FAT, EXT). With this project we move to `btrfs`. If you want to know why I like `btrfs` and decided to switch to that, read on.

## Btrfs can create a single pool from multiple disks
...
## Btrfs can create multiple subvolumes in a single pool
...
```
root@edison:~# /usr/bin/compsize -x /bin /boot /etc /home /lib /opt/ sbit /sketch/ /usr/ /var/
