...
The service does not harm, but of course you may disable it manually using `systemctl disable run-timezone.service` and call `run-timezone` later.  // run-timezone had no effect in yocto for me (@jessewriter)

command line alternative: # `timedatectl set-timezone America/Los_Angeles`  // find your timezone if not PST time zone
verity: # `timedatectl`
