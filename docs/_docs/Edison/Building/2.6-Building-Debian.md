...
```
And to start and create an Ubuntu docker image run the following.  Note if you are running off the Edison's internal memory you will use up all existing space and could freeze up so do not do this unless you are doing so on a sdhc card directly.
```
