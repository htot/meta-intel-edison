---
title: Working with bitbake
permalink: 5.1-Bitbake-tricks.html
sidebar: edison
product: Edison
---

Not everybody is familiar with `bitbake`. And although the giant `mega-manual` contains much of what there is to know here is small collection that I find useful but hard to remember.
## Bitbake build stats
The following creates similar to a systemd bootchart
```
poky/openembedded-core/scripts/pybootchartgui/pybootchartgui.py build/tmp/buildstats/<CUSTOM_BUILD_STATS_DIR>/
```
